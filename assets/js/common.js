...
var navMobile = {
  ...
  nav_mobile_down: function () {
    ...
    $('.nav-mobile .depth-1, .nav-mobile .depth-2').each(function () {
      if ($(this).children('').next().length > 0 && $(this).children().next().hasClass('d-none') == false ) {
        $(this).addClass('_down');
      } else {
  ...
  nav_mobile_action: function () {
    var $depth1 = $('.nav-mobile .depth-1'),
      ...
      $depth2_list = $('.nav-mobile .nav-list--depth2-wrap'),
    ...
    $depth2.children('.link').click(function () {
      if ($(this).next().length > 0 && $(this).next().hasClass('d-none') == false ) {
        if ($(this).next().css('display') === 'none') {
...
$(document).ready(function () {
  ...
  $('.nav, .nav-mobile').navActive({
    ...
    activeClass: 'on',
    callback: function () {
      $('.nav .dev-link, .nav-mobile .dev-link').removeClass('on');
  ...
  /* sub-body에 gnb-hover 붙이기*/
  if($('body').is('.login-body, .mypage-body') === true){
...
/* myhome__type nav 페럴랙스 */
// var mynav = $('#myhome .sub-nav-clone--depth3 .item');
// var cont = $('.myhome > section');

//  mynav.click(function (e) {
// 		e.preventDefault();
// 		var target = $(this);
// 		var myindex = target.index();
// 		//alert(index);
// 		var section = cont.eq(myindex);
...
//서브네비바 link에 on클래스
var nowhash = location.hash; //현재페이지 anchor값 불러오기
$('.finddatalink').find('a').each(function(){
  if ($(this).attr('data-link') == nowhash) { //data-link값고 현재 페이지의 앵커값이 같으면
    $(this).addClass('on');
  } else{
    $(this).removeClass('on');
  }
...
$(document).on('click','.noton',function(){ //클릭했을때 무조건 on클래스 붙게
  $(this).addClass('on');
});
